<x-app-layout>
    <div class="p-6 sm:p-8 max-w-5xl mx-auto space-y-6" x-data="{
        ...
        <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-dark-800/90 border border-gray-200/80 dark:border-dark-700 p-6 sm:p-8 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-gray-100 dark:border-dark-700">
                <div class="flex items-start gap-4">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-tr from-sky-500/20 to-indigo-500/20 border border-sky-500/30 text-sky-500 flex items-center justify-center font-black text-2xl flex-shrink-0 shadow-sm">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-dark-900 border border-gray-200/80 dark:border-dark-700 font-mono text-xs text-gray-600 dark:text-gray-300">
                                ...
                                <button type="button" @click="copySlug('{{ $category->slug }}')"
                                        class="text-gray-400 hover:text-sky-500 ml-1 transition" title="Copy slug">
                            ...
                            <span class="text-xs text-gray-500 dark:text-gray-400">
                                Created {{ $category->created_at?->format('F d, Y') }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 bg-gray-50/80 dark:bg-dark-900/60 p-4 rounded-2xl border border-gray-200/60 dark:border-dark-700/60 self-start sm:self-center">
                    <div class="text-center">
                        <div class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-sky-500 to-indigo-500">
        ...
        <div class="rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-950 border border-indigo-900/40 p-6 text-white shadow-sm">
            ...
            <p class="text-xs text-gray-400 leading-relaxed">
                When you build your frontend, this category and its associated posts can be fetched in JSON format by sending an <code class="text-sky-300">Accept: application/json</code> header to this resource.
            </p>
        </div>
        ...
        <div class="rounded-2xl bg-white dark:bg-dark-800/90 border border-gray-200/80 dark:border-dark-700 p-6 shadow-sm space-y-4">
            <div class="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-dark-700">
                <div>
                    <h2 class="text-lg font-bold text-gray-900 dark:text-white">Associated Blog Posts</h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Articles tagged under this category taxonomy</p>
                </div>
            ...
            @if($category->posts->isEmpty())
                <div class="py-12 text-center">
                    ...
                    <p class="text-sm font-semibold text-gray-800 dark:text-gray-200">No posts assigned yet</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-sm mx-auto">
                        Blog posts assigned to this category will appear here automatically.
                    </p>
                </div>
            @else
